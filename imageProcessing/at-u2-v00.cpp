...
/**
@page DocATu2v00 imageProcessing/at-u2-v00 
...
@b Usage: at-u2-v00 -i [input.pgm]

(for grey-level image restoration)

@b Usage: at-u2-v00 -i [input.ppm]
...
\code
./imageProcessing/at-u2-v00 -i ../imageProcessing/Images/degrade-b04.pgm --image-snr ../imageProcessing/Images/degrade.pgm -a 0.05 --epsilon-1 4 --epsilon-2 0.25 -l 0.0075 -p 2 -c 0xff0000 -o degrade
\endcode
...
int main( int argc, char* argv[] )
{
  ...
  if ( ! parseOK || vm.count("help") || !vm.count("input") )
    {
      cerr << "Usage: " << argv[0] << " -i toto.pgm\n"
           ...
           << endl
           << general_opt << "\n"
           << "Example: ./at-u2-v00 -i ../Images/cerclesTriangle64b02.pgm -o tmp -a 0.05 -e 1 --lambda-1 0.1 --lambda-2 0.00001 -g"
           << endl;
      return 1;
    }
  ...
  KSpace K;
  ATu2v00< KSpace > AT( verb );
  Domain domain;
  AT.setMetricAverage( metric );
  
  typedef ATu2v00<KSpace>::Calculus Calculus;
