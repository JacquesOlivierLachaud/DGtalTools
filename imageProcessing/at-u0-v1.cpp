...
int main( int argc, char* argv[] )
{
  ...
  double ar  = vm[ "alpha-r" ].as<double>();
  if ( vm.count( "alpha" ) ) a1 = a2 = vm[ "alpha" ].as<double>();
  if ( a2 > a1 ) a2 = a1;
  if ( ar <= 1.0 ) ar = sqrt(2);
  ...
  ostringstream ossF;
  ossF << boost::format("%s.txt") % f2;
  //const string file = f2 + ".txt";
  const string file = ossF.str();
      ...
      if ( grey_image )
        {
          ...
          ossU << boost::format("%s-a%.5f-l%.7f-u0.pgm") % f2 % a1 % l1;
          ossV << boost::format("%s-a%.5f-l%.7f-u0-v1.pgm") % f2 % a1 % l1;
          ossW << boost::format("%s-a%.5f-l%.7f-u0-v1.ppm") % f2 % a1 % l1;
...
//          PGMWriter<GreyLevelImage>::exportPGM( ossV.str(), image_uv );
      ...
      else if ( color_image )
        {
          ...
          ossU << boost::format("%s-a%.5f-l%.7f-u0.ppm") % f2 % a1 % l1;
          ossV << boost::format("%s-a%.5f-l%.7f-u0-v1.ppm") % f2 % a1 % l1;
