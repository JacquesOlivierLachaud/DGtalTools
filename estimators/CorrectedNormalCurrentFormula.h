...
namespace DGtal
{
  ...
  /**
     ...
     MU2=1/2*( uAx*( uBy*uCz - uCy*uBz ) -uAy*( uBx*uCz - uCx*uBz )+uAz*( uBx*uCy - uCx*uBy ) = 1/2*det( uA, uB, uC )
  ...
  */
  ...
  class CorrectedNormalCurrentFormula
  {
    ...
    /// Computes area of triangle abc given a constant
    /// corrected normal vector \a u.
    static
    Scalar areaConstantU
    ( const RealPoint& a, const RealPoint& b, const RealPoint& c,
      const RealVector& u )
    {
      return ( b - a ).crossProduct( c - a ).dotProduct( u );
    }

    /// Computes area of triangle abc given an interpolated
    /// corrected normal vector \a ua, \a \ub, \a uc.
    static
    Scalar areaInterpolatedU
    ( const RealPoint& a, const RealPoint& b, const RealPoint& c,
      const RealVector& ua, const RealVector& ub, const RealVector& uc )
    {
      // mu0 =  1/6 < (u_A + u_B + u_C ) | (AB x AC ) >
      return 1.0/6.0 * ( b - a ).crossProduct( c - a ).dotProduct( ua + ub + uc );
    }
    
    /// Computes area of polygonal face \a pts given a constant
    /// corrected normal vector \a u.
    static
    Scalar areaConstantU( const RealPoints& pts, const RealVector& u )
    {
      if ( pts.size() <  3 ) return 0.0;
      if ( pts.size() == 3 )
	return areaConstantU( pts[ 0 ], pts[ 1 ], pts[ 2 ], u );
    ...
    /// corrected normal vector \a ua, \a \ub, \a uc.
    static
    Scalar areaInterpolatedU( const RealPoints& pts, const RealVectors& u )
    {
      if ( pts.size() <  3 ) return 0.0;
      if ( pts.size() == 3 )
	return areaInterpolatedU( pts[ 0 ], pts[ 1 ], pts[ 2 ],
				  u[ 0 ], u[ 1 ], u[ 2 ] );
      const RealPoint   b = barycenter( pts );
      const RealVector ub = barycenter( pts );
      Scalar          a = 0.0;
      for ( Index i = 0; i < pts.size(); i++ )
	a += areaInterpolatedU( b,  pts[ i ], pts[ (i+1)%pts.size() ],
				ub,   u[ i ],   u[ (i+1)%pts.size() ] );
      return a;
    }
